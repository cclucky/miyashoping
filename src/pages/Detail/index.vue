    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImageLists="skuImageLists" />
          <!-- 小图列表 -->
          <ImageList :skuImageLists="skuImageLists" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  class="itxt"
                  v-model="subrsNum"
                  @change="changeSubrsNum"
                />
                <a href="javascript:" class="plus" @click="subrsNum++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="subrsNum <= 1 ? (subrsNum = 1) : subrsNum--"
                  >-</a
                >
              </div>
              <div class="add">
                <a href="javascript:" @click="addBuyShoopCars">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeColor(spuSaleAttrValue, arr) {
      arr.forEach((item) => {
        item.isChecked = "0";
      });
      spuSaleAttrValue.isChecked = "1";
    },
    changeSubrsNum(event) {
      let value = event.target.value * 1;
      if (isNaN(value) || value < 1) {
        this.subrsNum = 1;
      } else {
        this.subrsNum = parseInt(value);
      }
    },
    async addBuyShoopCars() {
      try {
        await this.$store.dispatch("addBuyShoopCar", {
          skuId: this.$route.params.skuid,
          skuNum: this.subrsNum,
        });
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.subrsNum}})
      } catch (error) {
        alert(error.message);
      }
    },
